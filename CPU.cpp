...
bool CPU::get_flag(CpuFlags f) const { return (registers.p & f); }
...
void CPU::init_opcodes() {
  ...
  opcodes->emplace_back(new OpCode(0xd0,
                                   "BNE",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0x70,
                                   "BVS",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0x50,
                                   "BVC",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0x30,
                                   "BMI",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0xf0,
                                   "BEQ",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0xb0,
                                   "BCS",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0x90,
                                   "BCC",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  opcodes->emplace_back(new OpCode(0x10,
                                   "BPL",
                                   2,
                                   2 /*(+1 if branch succeeds +2 if to a new page)*/,
                                   AddressingMode::NoneAddressing));
  ...
  // opcodes->emplace_back(new OpCode(0xCB, "IGN", 3,4 /* or 5*/, AddressingMode::Absolute_X));
  ...
  opcodes->emplace_back(new OpCode(0xab, "*LXA", 2, 3, AddressingMode::Immediate)); //todo: highly unstable and not used
  //http://visual6502.org/wiki/index.php?title=6502_Opcode_8B_%28XAA,_ANE%29
  opcodes->emplace_back(new OpCode(0x8b, "*XAA", 2, 3, AddressingMode::Immediate)); //todo: highly unstable and not used
  ...
  //todo: highly unstable and not used
  opcodes->emplace_back(new OpCode(0x9f, "*AHX", 3, /* guess */ 4/* or 5*/, AddressingMode::Absolute_Y));
  //todo: highly unstable and not used
  opcodes->emplace_back(new OpCode(0x9e, "*SHX", 3, /* guess */ 4/* or 5*/, AddressingMode::Absolute_Y));
  //todo: highly unstable and not used
  opcodes->emplace_back(new OpCode(0x9c, "*SHY", 3, /* guess */ 4/* or 5*/, AddressingMode::Absolute_X));
  //todo: highly unstable and not used
...
const OpCode* CPU::get_opcode(uint8_t code) const {
