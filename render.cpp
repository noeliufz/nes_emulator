    ...
    for (auto i = 0; i < 0x3c0; ++i)
    {
        ...
        for (auto y = 0; y <= 7; ++y)
        {
            ...
            for (int x = 7; x >= 0; --x)
            {
                ...
                auto pixel_x = tile_column * 8 + x;
                auto pixel_y = tile_row * 8 + y;
                if (pixel_x >= view_port.x1 && pixel_x < view_port.x2 && pixel_y >= view_port.y1 &&
                    pixel_y < view_port.y2)
                {
                    frame.set_pixel(static_cast<size_t>(shift_x + static_cast<int>(pixel_x)),
                                    static_cast<size_t>(shift_y + static_cast<int>(pixel_y)), rgb);
                }
            }
        }
    }
...
void render(const NesPPU &ppu, Frame &frame)
{
    ...
    render_name_table(ppu, frame, main_nametable, r, -static_cast<int>(scroll_x), -static_cast<int>(scroll_y));

    if (scroll_x > 0)
    {
        Rect r{0, 0, scroll_x, 240};
        render_name_table(ppu, frame, second_nametable, r, static_cast<int>(256 - scroll_x), 0);
    }
    else if (scroll_y > 0)
    {
        Rect r{0, 0, 256, scroll_y};
        render_name_table(ppu, frame, second_nametable, r, 0, static_cast<int>(240 - scroll_y));
    }
