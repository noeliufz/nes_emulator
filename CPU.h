...
struct OpCode {
  ...
  AddressingMode mode;
  // constructor
  OpCode(int i_code, const char* str, int i_len, int i_cycles, AddressingMode addressing_mode): code(i_code),
    mnemonic(str), len(i_len), cycles(i_cycles), mode(addressing_mode) {
  }
};
...
class CPU {
  public:
    ...
    std::vector<const OpCode*>* opcodes;
    // Linkage with bus
    EM::Bus* bus = nullptr;
